...
{
    struct scte35_event* event = (struct scte35_event*)av_malloc(sizeof(struct scte35_event));
    if (!event)
        return NULL;
...
struct scte35_interface* ff_alloc_scte35_parser(void *parent, AVRational timebase)
{
    ...
    struct scte35_interface* iface = (struct scte35_interface*)av_mallocz(sizeof(struct scte35_interface));
    if (!iface)
       return NULL;
...
{
    struct scte35_interface* iface = (struct scte35_interface*)avctx->priv_data;
    return 0;
}
...
static int decode(AVCodecContext *avctx, void *output, int *outdata_size, AVPacket *avpkt) {
    struct scte35_interface* iface = (struct scte35_interface*)avctx->priv_data;
